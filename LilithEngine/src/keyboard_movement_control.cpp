...
namespace lth {
	...
	void KeyboardMovementController::moveInPlaneXZ(
		...
		if (glm::dot(rotate, rotate) > std::numeric_limits<float>::epsilon()) {
			glm::vec3 rotation = transform.getRotationEuler() + lookSpeed * dt * glm::normalize(rotate);
			rotation.y = PI - glm::clamp(glm::mod(PI - rotation.y, TWO_PI), PI_OVER_TWO, THREE_PI_OVER_TWO);
			transform.setRotationEuler(rotation);
		}

		const glm::vec3 forwardDir = transform.getForward();
		...
		if (glm::dot(displacement, displacement) > std::numeric_limits<float>::epsilon()) {
			transform.addTranslation(moveSpeed * dt * glm::normalize(displacement));
		}
